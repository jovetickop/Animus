...
session=$(echo "$input" | grep -o '"session_name":"[^"]*"' | head -1 | sed 's/"session_name":"//;s/"//' | cut -c1-8)
[ -z "$session" ] && session=$(echo "$input" | grep -o '"session_id":"[^"]*"' | head -1 | sed 's/"session_id":"//;s/"//' | cut -c1-8)
